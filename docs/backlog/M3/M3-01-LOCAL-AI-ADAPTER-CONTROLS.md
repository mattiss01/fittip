...
**Fixtures are an authored checklist, not sampled output.** Enumerate each
failure mode deliberately: truncated JSON, schema-violating JSON, impossible
date or duration, unowned goal reference, oversized payload, extra fields,
unsafe medical phrasing. Never generate malformed output from a model.
