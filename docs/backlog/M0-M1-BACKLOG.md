...
basic-auth M0-03, the local-owner AI staging principle in ADR-006, and the
founder-hosted topology in ADR-007 are accepted. M0-06A is testable and awaits
product-owner acceptance.
M0-03B, M0-04, M0-05, and M0-06 remain proposed pre-friends/public/commercial
gates. Independently approved M1/M2 tickets may proceed locally, and after
M0-06A acceptance in founder staging, with only the product owner or synthetic
data; no friend data, public registration, commercial use, analytics sink,
provider/model/key, or spend is authorized by this staging change.
...
| P0 | [M0-06A Founder-hosted staging](M0-06A-FOUNDER-HOSTED-STAGING.md) | testable | M0-03 and M0-02-C1 accepted; ADR-007 accepted | Free Vercel plus separate Frankfurt Supabase founder staging; hosted signup closed; owner UUID allowlist; disposable owner/synthetic data; migration/RLS/390px hosted validation | Approved by product owner on 26 July 2026; Terra implementation and independent review passed; hosted validation is recorded; product-owner acceptance is required before hosted M1 |
...
| P1 | [M1-01 Goal model and validation](M1-01-GOAL-MODEL-VALIDATION.md) | proposed | M0-06A accepted for hosted testing; M0-03 and M0-02-C1 accepted | Sport-agnostic goal CRUD; lifecycle; separate core/supporting ranks; maximum three active core goals; ownership/RLS; concurrency and archive/delete rules; 390px management | Approve goal fields, lifecycle/rank, archive/delete, concurrency architecture, mobile UX/copy, and privacy classification before implementation |
| P1 | [M1-02 Memory model and management](M1-02-MEMORY-MODEL-MANAGEMENT.md) | proposed | M0-06A accepted for hosted testing; M0-03 and M0-02-C1 accepted | Explicit facts, constraints, preferences, and proposed patterns; provenance/status/history; inspect/edit/disable/delete; ownership/RLS; sensitive-data handling; 390px management; no AI extraction | Approve statuses, provenance, history/expiry/delete, sensitive-data handling, mobile UX/copy, and consequential data architecture before implementation |
...
[ADR-006](../decisions/ADR-006-LOCAL-OWNER-AI-MVP.md) preserves the local
owner/synthetic path. [ADR-007](../decisions/ADR-007-FOUNDER-HOSTED-STAGING.md)
and M0-06A add only a disposable owner/synthetic hosted path. Hosted M1
therefore waits for M0-06A acceptance plus each ticket's own dependencies.
M0-03B, M0-04 and its later implementation, M0-05, and M0-06 are still
required before friend data, public registration, commercial use, or
production.
