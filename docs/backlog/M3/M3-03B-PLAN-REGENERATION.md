...
its "Split into three tickets" section for the reasoning and the boundary.
...
repeating itself and preserve what was not criticized.
...
Never the chain. Context size stays flat however many rounds the owner goes.
Every round is still persisted as immutable evidence; storage and transmission
are separate concerns and this ticket keeps them separate.

## The cap

**Regeneration is capped at 3 per horizon**, decided 9 August 2026. At the cap
the owner is directed to M3-04's editing rather than left stuck.
...
1. A regeneration carries exactly one superseded proposal to the boundary,
   however many rounds have run, and context size does not grow across rounds.
...
7. The copy at the cap reads as a suggestion rather than a punishment, and a
   disabled control never appears without an honest reason.
...
- Exactly one superseded proposal reaches the boundary at rounds 1, 2, and 3;
  assert the context byte size does not grow across them.
...
1. **The copy shown when the cap is reached**, and what the owner sees. At the
   cap they are directed to M3-04's editing, and that transition has to read as
   a suggestion rather than a punishment. (Originally decision 14's open half;
   the cap of 3 itself is decided.)
2. **Whether regeneration may change the day count or start date**, or must
   reuse the original horizon. Recommendation: **reuse** — changing the dates
   makes it a new request rather than a regeneration, and the superseded
   proposal would then be about different days. (Originally decision 15.)
3. **Copy and layout for the two compose fields**, now that regeneration carries
   a prefilled planning note **and** a required feedback field. The owner has to
   ...
   forbidden from reading them as constraints. (Originally decision 16.)
