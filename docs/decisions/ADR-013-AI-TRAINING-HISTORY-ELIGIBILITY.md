...
2026-09-26"), under the same 500-character truncation. Every one of those three
facts is already sent for the linked log itself; the line only says which log
replaced which, so eligibility does not change. A replaced log written before
