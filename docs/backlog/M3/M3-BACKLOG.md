...
| P1 | [M3-12 Manual continuous planning](M3-12-MANUAL-CONTINUOUS-PLANNING.md) | in development | M3-11 accepted and founder reset recorded; M3-18 accepted and merged | Restore `/home/plan` directly with one-off Plan changes, Recovery day labels, the owner-local past boundary and 10-per-date cap enforced server-side, locks, and conflicts. No visible plan-history surface | Contract drafted and Tier 1 dispatch approved 17 Aug 2026. Adds `profiles.timezone_name` and `rolling_plan_recovery_days`, and replaces `apply_rolling_plan_change_set` to enforce the two rules M3-10 left out |
