...
| P1 | [M2-11 Upgrade to `next@16.3.0` and re-measure](M2-11-NEXT-16-3-0-UPGRADE.md) | accepted | M2-09 accepted | Take the upgrade carrying the upstream `useDeferredValue` fix; re-measure all four transitions on M2-09's probe, measure `/home/plan`'s own save for the first time, decide the React pins, and remove a mitigation or the CI retries only where the after-measurement supports it | Accepted 13 August 2026 at `7585f1f`. `/home/plan` navigation 9/250 to 0/250 on a paired measurement; the CI retry stopgap removed, the four watchdog surfaces kept |
...
[M2-11](M2-11-NEXT-16-3-0-UPGRADE.md), accepted on 13 August 2026. The
hypothesis held: `/home/plan` navigation went 9/250 to 0/250 against a control
run that still reproduced the defect an hour earlier. The four watchdog surfaces
keep their recovery, because `watchTransition` is armed by mutation timestamps
and structurally cannot fire on a navigation — a clean navigation figure is not
weak evidence about them but none. It carries a falsifiable hypothesis rather than a task list: if
