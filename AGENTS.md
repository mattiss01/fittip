...
- A Vercel `READY` state and green CI do not prove that a hosted database migration ran. Before handing any Preview that depends on new migrations to the reviewer or product owner, the lead applies the exact committed migrations to the founder Supabase project in timestamp order, confirms that remote migration history contains the repository's exact versions, verifies the expected schema plus RLS/privilege boundary and advisors, exercises the affected authenticated hosted read path, and records that evidence in the ticket validation record. A missing migration, history drift, or unverified authenticated read is a delivery blocker; do not request acceptance.
- The reviewer reviews the exact pushed commit, not an uncommitted working tree, reconciles the manifest, confirms the matching Vercel Preview reached `READY`, and does not need to operate a browser or perform the hosted UI pass. Review corrections create new ticket-branch commits, are pushed again, and invalidate approval of earlier commits and previews until re-reviewed.
- Product-owner acceptance is requested against the exact independently reviewed commit and its Vercel Preview URL. The product owner owns the manual hosted interaction and visual check and may attest its result directly; the reviewer never has to repeat that browser run. The product owner's manual acceptance surface is Vercel, not localhost.
...
- The green CI run for the exact reviewed commit is the automated-test evidence for that ticket. Record its run URL in the validation record instead of pasting suite output. A red or absent run for that SHA is a delivery blocker, subject only to the two exceptions below.
- **Known-defect exception.** A red run does not block when every failure in it is attributable to a named, already-ticketed defect that fails identically on unchanged `master`. The run must be green on everything the ticket touches; a failure anywhere the ticket changed behavior is a regression and blocks regardless of what else is open. Claiming the exception requires naming the ticket, linking both the failing run and a run showing the same failure without the ticket's changes, and recording all of it in the validation record. The product owner accepts the exception explicitly, as a separate decision from accepting the work. Never widen it to a failure nobody has yet diagnosed: an undiagnosed failure is a blocker precisely because no one can say what it means.
- **Evidence-commit exception.** A commit that changes only the validation record does not require its own recorded run. A record cannot contain the SHA or run URL of the commit that adds it, so recording each run in a further commit never terminates. The lead verifies that final run and records its result when acceptance is recorded.
- These exceptions exist so the gate stays honest rather than becoming something everyone routes around. A gate that blocks on a failure it cannot distinguish from a known one teaches every reader to ignore it, which is worse than having no gate. That is also why both exceptions demand a written, checkable justification rather than a judgement call.
