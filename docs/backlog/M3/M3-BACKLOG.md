...
| P1 | [M3-15A Replacement completion foundation](M3-15A-COMPLETION-FOUNDATION.md) | accepted | M3-14B accepted and merged | One owner-editable completion, its activity snapshot, the immutable planned snapshot, the owner-derived write function, the completion-blocks-deletion rule over M3-14's two sweeping functions, and the ADR-017 top-up wrapper. **No UI**, on M3-10 and M3-14's precedent | Split out of M3-15 on 29 Aug 2026 because the combined scope could not fit a 40-line brief. Tier 1 dispatch approved 29 Aug 2026 against [the F-005 and ADR-013 owner-mutable-completion amendments](../../decisions/ADR-013-AI-TRAINING-HISTORY-ELIGIBILITY.md): no revision chain, no `completion_heads`, no `correction_reason`, and no `rest` status. The first CI run was red because the dispatched brief reused the retired `completed_activities` name; the lead corrected the brief in `e56fcfc` and the table is `completion_activities`. Accepted 29 Aug 2026 against independently reviewed `0cc8d46`. Founder migration applied and fully verified: history aligned at all 19 positions, one new advisor warning in the existing ADR-008 category, privilege boundary confirmed, and the authenticated/anon read pair as specified. Unblocks M3-15 |
| P1 | [M3-19 Delete a planned session](M3-19-DELETE-A-PLANNED-SESSION.md) | accepted | M3-15A accepted | A third session verb: hard-delete one planned session beside the existing cancel, refused when a completion exists, and the card relabelled from the ambiguous "Remove" to **Cancel** + **Delete** | Product owner approved the verb and the relabelling on 29 Aug 2026 against [the F-005 amendment](../../product/F-005-ROLLING-TRAINING-PLAN.md#deleting-a-planned-session-29-august-2026). Tier 1 dispatch approved the same day. Sequenced before M3-15B so Today inherits the corrected verbs . Accepted 30 Aug 2026 against `43a3140` after six review rounds; merged as `bc553e9`. Founder migration `20260829135426` applied and history verified. Unblocks M3-15B |
| P1 | [M3-15 Replacement consumer readiness](M3-15-REPLACEMENT-CONSUMER-READINESS.md) | **split — do not dispatch** | — | Retained as the record of the narrowed consumer scope and its reasoning; the work is M3-15B through M3-15E | Split a second time on 29 Aug 2026. Keeps its number because ADR-017, F-005, and M3-16 already say "M3-15" to mean Today, Progress, and AI context |
| P1 | [M3-15B Today and logging](M3-15B-TODAY-AND-LOGGING.md) | accepted | M3-19 accepted | Restore `/home/today` and `/home/log`, the logging path onto M3-15A's write function including **skip**, the first `readPlanWindowToppedUp` caller, and `completedKept` on the series receipt | Tier 2. Agent brief and dispatch approved by the product owner on 30 Aug 2026. Today lists one owner-local day with day navigation; `/home/log` writes. Round 3 dispatched 31 Aug 2026 from the product owner Preview pass: an unplanned log gains a required title and sport, and a skipped outcome drops duration, effort, and feeling. Accepted 31 Aug 2026 against `88b3d84`; merged as `fa063e2`. No founder migration needed - no schema changed. Two deferred changes filed as M3-23 items 2 and 3. Unblocks M3-15C |
| P1 | [M3-15C Progress](M3-15C-PROGRESS.md) | accepted | M3-15B accepted | Restore `/home/progress` and `/home/progress/[id]`: a calendar month of the training record, and one completion beside the immutable planned snapshot it was measured against | Tier 2. Agent brief written 31 Aug 2026 against the product owner scope decisions of that date: completions only, no computed figures. Dispatch approved by the product owner on 31 Aug 2026; builder spawned onto `ticket/m3-15c-progress` the same day. Accepted 14 Sep 2026 against independently reviewed `0d2dfbc` after three review rounds; merged as `42debea`. No hosted migration needed - nothing under `supabase/` changed. Plan change history split out as M3-24. Unblocks M3-15D |
| P1 | [M3-15D Bounded AI completion context](M3-15D-AI-COMPLETION-CONTEXT.md) | accepted | M3-15C accepted | Build the production `CoachAIContextSource`, which does not exist — M3-11 deleted the legacy adapter and left a bare interface. Bounded completion history through the ADR-013 allowlist, plus M3-08's exact-source rule | Tier 1: crosses the ADR-013 boundary. Agent brief written 14 Sep 2026 against the code as it stands; Tier 1 dispatch approved by the product owner the same day with five recorded decisions — the data boundary approved as the allowlist defines it including the four health signals, fixture-only with no live provider call, `correctionReason` deleted, the ADR-017 top-up side effect accepted, and `actualStartedAt` deliberately left out of the allowlist. Accepted 14 Sep 2026 against independently reviewed `ffe3c7e` after two review rounds, whose CI run 34833095545 is green on its first attempt; merged as `85567a9`. No hosted migration needed. The M3-22 exception accepted for this ticket went unused. Tripwire coverage follow-up split out as M3-25 |
| P1 | [M3-15E Roadmap surface, read-only](M3-15E-ROADMAP-RESTORATION.md) | accepted | M3-15D accepted | Restore `/home/plan/roadmap` from its maintenance stub as a read of the current roadmap, its history, and proposals as records — including the ones M3-11 force-expired | Tier 2. Rescoped on 14 Sep 2026: the measured deletion is 4,239 lines, not 1,300, and four dependencies of the deleted surface no longer exist, so this is a rewrite rather than a revert. All five revoked functions are mutations and every roadmap read goes at the tables under RLS, so this slice needs no migration and no re-grant. Agent brief written 14 Sep 2026; Tier 2 dispatch approved by the product owner the same day, builder spawned onto `ticket/m3-15e-roadmap-read`. Accepted 15 Sep 2026 against independently reviewed `693bd46` after three review rounds, with the owner's hosted read of a real roadmap on the Preview; merged as `3570c25`. No hosted migration needed. Two deferred copy defects (uncertainty labels, inlined M3-02 wordings) carried on M3-15F. Unblocks M3-15F, M3-16 and M3-20 |
| P1 | [M3-15F Roadmap generation and privilege re-grant](M3-15F-ROADMAP-GENERATION.md) | accepted | M3-15E accepted | Re-grant the five roadmap RPCs M3-11 revoked, and wire generation, regeneration, edit, decline and acceptance through `coach-ai-service` | Tier 1: privilege re-grant plus a hosted founder migration the lead cannot produce. Split from M3-15E on 14 Sep 2026 so the privilege arrives with its caller. Brief written and Tier 1 dispatch approved 16 Sep 2026 on six recorded decisions: no live provider call (fixture adapter only), controls visible and working without a credential with fixture-authored records labelled examples, acceptance kept in scope, M3-09 fixed in the same migration, one ticket despite an 82-line brief, and the hosted apply shipped as CLI commands rather than a runbook or a script. Accepted 18 Sep 2026 against independently reviewed `a1cc456` after three review rounds: round one rejected `2fb0755` (untested source recheck, a record claiming an authorization the stale branch copy of the brief did not carry), round two rejected `ba665b0` (a freshly-mounted control inherited the previous write's reply and falsely declared the next write lost), round three approved. Hosted migration applied by the product owner 17 Sep 2026: remote history carries `20260916075522`, advisors clean by the M3-15A criterion. Unblocks M3-16 |
...
| P2 | [M3-23 Completion write follow-ups](M3-23-COMPLETION-WRITE-FOLLOW-UPS.md) | proposed | M3-15B accepted | Two things the logging surface cannot fix from where it stands: a duplicate log refused with the wrong reason, and an unplanned log title that cannot be corrected after it is written | Tier 1 — both change the accepted `apply_completion_change` and need a forward migration. Filed by the lead on 31 Aug 2026 from M3-15B. **Needs a decision before dispatch**: whether item 2 justifies a Tier 1 migration, and whether the two ship together |
| P2 | [M3-25 Network tripwire coverage](M3-25-NETWORK-TRIPWIRE-COVERAGE.md) | proposed | none | The coaching network tripwire enumerates the roots it scans and claims the list is complete, but does not watch `src/server/completions` or `src/server/rolling-plan`, both of which shape provider-bound owner data | Tier 3, test only. Raised by the M3-15D round 2 reviewer on 14 Sep 2026; the product owner chose to carry it as a follow-up rather than reopen that ticket. Neither root contains a network primitive today, so this is a control overstating its coverage rather than a live hole. One open design question: whether every root earns a path pin |
| P2 | [M3-24 Plan change history](M3-24-PLAN-CHANGE-HISTORY.md) | proposed | M3-15C accepted | F-005 Review history items 2 and 3: plan history organized by understandable changes, each opening to show the affected sessions and their before/after values | Tier 2, split from M3-15C on 31 Aug 2026. The tables are already granted and RLS-confined, so no migration is expected, but no read path exists. **Needs decisions before dispatch**, chiefly how ADR-017 machine-provenance change sets are shown |
                            ...
                            -> M3-15B Today and logging
                              -> M3-15C Progress
                                -> M3-15D bounded AI completion context
                                  -> M3-15E roadmap surface, read-only
                                    -> M3-20 reactivate a cancelled session
                                    -> M3-15F roadmap generation + re-grant
                                      -> M3-16 AI proposal application (+ M3-03C)
                                        -> rewritten M3-03B regeneration
                                          -> M3-17 final rolling-plan closeout

M3-20 and M3-15F are both sequenced after M3-15E and neither blocks the other,
...
rule moves into M3-15D, and M3-17 replaces M3-05's duplicate validation pass.
