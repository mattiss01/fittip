...
| P1 | [M3-10 Rolling-plan foundation](M3-10-ROLLING-PLAN-FOUNDATION.md) | testable | F-005 approved; ADR-016 accepted; one-cutover exception authorized | Dormant owner-scoped rolling-plan identity, current planned-session state, atomic before/after change log, owner revision, RLS/grants/indexes, and concurrency; no UI, activation, old-data mutation, or AI | Independently approved with exact-commit CI, matching Preview, and founder Supabase migration/security evidence on 14 Aug 2026; product-owner acceptance remains required |
